...
`htrSPRanalysis` can be installed using `devtools::install_git`. If you do not already have `devtools` installed, you can install it using `install.packages(devtools)`. Once devtools is installed, install the package via:

```{r eval=FALSE}

devtools::install_git("https://gitlab.oit.duke.edu/janice/htrSPRanalysis",
                      build_vignettes = TRUE, ref = "e7709231d43b79b868df08e4a7d6b10cf11f2bdc")
...
# process the input
processed_input <- process_input(files_directory,
                    sample_sheet_path,
                    data_file_path,
                    num_cores = 2)
...
bulkshift_example <- process_input(files_directory = files_directory,
